...
class 页面逻辑(逻辑基类):
    ...
    def 运行结束(cls, 参数: 共享参数) -> Any:
        ...
        cls.log.info(f"已处理页面: {len(已处理页面)}, 章节页面数量: {len(章节页面数量)}")
    ...
    def 循环任务(cls, 参数: 共享参数) -> Any:
        ...
        def 请求页面():
            cls.log.info(f"开始请求页面：{页面链接}")
            响应 = requests.get(页面链接, timeout=5)
            if 响应.status_code == 200:
                cls.log.info(f"页面请求成功：{页面链接}")
                cls.处理响应内容(响应, 页面下标, 页面链接, 参数)
                参数.页面字典[页面链接] = 2
                return True
            else:
                cls.log.warning(f"请求页面 {页面链接} 失败，状态码: {响应.status_code}")
                return False
    ...
    def 处理响应内容(cls, 响应, 页面下标, 页面链接, 参数):
        ...
        cls.log.info(f"找到 {len(下一页链接)} 个下一页链接")
        新页面下标 = 页面下标
        for 页面 in 下一页链接:
            ...
            if 完整链接 not in 参数.页面字典:
                ...
                cls.log.info(f"新页面链接已添加到队列：{完整链接}")

        分页章节下标计数 = 0
        章节列表 = 网页.xpath('/html/body/div[4]/ul/li/a')
        cls.log.info(f"找到 {len(章节列表)} 个章节")
        for 章节 in 章节列表:
            章节链接 = f"http://m.biqu5200.net{章节.get('href')}"
            章节标题 = 章节.text
            cls.log.info(f"新章节已添加：{章节标题}")
            参数.章节字典[章节标题] = 1
            分页章节下标计数 += 1
        ...
        try:
            ...
            cls.log.error(f"追加写入记录文件 出现错误: {页面链接} == {异常}")
